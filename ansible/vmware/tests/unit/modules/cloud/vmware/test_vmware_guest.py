...
from ansible_collections.ansible.vmware.plugins.modules import vmware_guest
...
def test_vmware_guest_with_parameters(mocker, capfd, testcase):
    if testcase.get('test_ssl_context', None):
        class mocked_ssl:
            pass
        mocker.patch('ansible_collections.ansible.vmware.plugins.module_utils.vmware.ssl', new=mocked_ssl)
